...
export default function Occupation() {
  ...
  const fetchOccupations = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/reference/occupations`, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/reference/occupations/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const result = await response.json();
  ...
  const handleToggleActive = async (id: string, name: string, currentActive: boolean) => {
    try {
      await fetch(`${API_BASE_URL}/reference/occupations/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
  ...
  return (
    <AdminLayout title="Occupation Management">
