...
export default function City() {
  ...
  const fetchCities = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/reference/cities`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const result = await response.json();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/reference/cities/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const result = await response.json();
  ...
  const handleToggleActive = async (id: string, name: string, stateId: string, currentActive: boolean) => {
    try {
      await fetch(`${API_BASE_URL}/reference/cities/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
